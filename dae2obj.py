...
def convert_dae_to_obj(input_path, output_path, overwrite=False):
    """
    转换单个DAE文件到OBJ
    """
...
def batch_convert_dae_to_obj(input_dir, output_dir=None, overwrite=False, recursive=True):
    """
    批量转换目录下的所有DAE文件
    """
    if not output_dir:
    ...
    for root, dirs, files in os.walk(input_dir):
        for filename in files:
            if filename.lower().endswith('.dae'):
                dae_path = os.path.join(root, filename)
                ...
                convert_dae_to_obj(dae_path, obj_path, overwrite)
                
        if not recursive:
            break

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="批量将DAE文件转换为OBJ格式")
    parser.add_argument("input_dir", help="包含DAE文件的输入目录")
